...
**Hoy son seis integradas de las 61 registradas** —BOE, DOGC, BOA, BOCYL, BOCM y BOPV (ADR
0019, 0028, 0029, 0034 y 0035)—, y cada una ha traído su propia forma de romper: el DOGC mete el
articulado dentro de un atributo XML, el BOA sirve su portada los días sin boletín, el BOCYL
obliga a raspar HTML, el BOCM repite su sumario entero 37 veces y llama `fecha_publicacion` al
día anterior, y **el BOPV publica dos boletines el mismo día unas cinco veces cada 33 meses**.
...
- ~~Más de 5 fuentes en la primera iteración.~~ — **ampliado a 6 por el humano el 2026-09-06**:
  ...
  esta sección protege sigue siendo el plazo: **6 y se para** —agotado con el BOPV (ADR 0035)—,
  y la séptima necesita otra decisión tuya, no otra migración.
...
- **ADRs** en `docs/adr/NNNN-titulo.md`. Formato: contexto, decisión, alternativas,
  consecuencias. **Están todos escritos del 0001 al 0035** y su título dice de qué van: `ls
  docs/adr/` es el índice, y duplicarlo aquí solo creaba dos listas que se desincronizan.
  **El siguiente número libre es el 0036.** No queda ninguno reservado.
...
# Ingesta manual (una fecha concreta). Las seis fuentes integradas:
#   boe | dogc | boa | bocyl | bocm | bopv   (la tabla FUENTES de worker/run.py manda)
